...
public class Solution657 {
    ...
    public static boolean test(String s) {
        ...
        int countRightAndLeft = 0;

        for(int i =0;i<s.length();i++){
            switch (s.charAt(i)){
                case 'U':countUpAndDown++;
                continue;
                case 'D':countUpAndDown--;
                continue;
                case 'R':countRightAndLeft++;
                continue;
                case 'L':countRightAndLeft--;
                continue;
            }

        }if(countRightAndLeft==0&&countUpAndDown==0){
            return true;
        } else return false;
    }
